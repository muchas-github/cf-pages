...
// CSS related plugins.
	...
	rtlcss       = require( 'gulp-rtlcss' ), // Generates RTL stylesheet.
 ...
 *    1. Take HTML source files from source folder
 *    2. Cretae relevant folder in root for minified version
 *    3. Minify HTML and moves to relevant folder
 */
 gulp.task( 'htmlMin', () => {
	return gulp
		.src( config.htmlSourceSRC,  { allowEmpty: true } ) // Only run on changed files.
		.pipe(htmlmin({ collapseWhitespace: true }))
    	.pipe( gulp.dest( config.htmlDestSRC ) )
		.pipe(
			notify( {
				message: '\n\n✅ HTML Minified!\n',
				onLast: true
...
/**
 * Task: `inlinie critical abovethe fold css`.
 *
*/ 
gulp.task('critical', () => {
	return gulp
	  .src(config.htmlSourceSRC)
	  .pipe(
		critical({
		  base: './',
		  inline: true,
 ...
 * Runs specific tasks.
 */
gulp.task( 'run', gulp.parallel( 'styles', 'vendorsJS', 'globalJS', 'customJS', 'images', 'htmlMin' ) );
...
gulp.task(
	'watch',
	gulp.parallel( 'htmlMin', 'styles', 'vendorsJS', 'globalJS', 'customJS', 'images', () => {
		...
		gulp.watch( config.watchHTMLMin, gulp.parallel( 'htmlMin' ) );
...
gulp.task(
	'default',
	gulp.parallel( 'htmlMin', 'styles', 'vendorsJS', 'globalJS', 'customJS', 'images', browsersync, () => {
